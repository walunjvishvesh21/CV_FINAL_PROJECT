...
# Baseline experiment here we will train and evaluate an ImageNet-pretrained ResNet50 
# on the selected 6-class GTSRB subset using only original data.


def set_seed(seed=42):
    # we are Setting random seeds for reproducibility so training results are more consistent across runs.
    random.seed(seed)
    np.random.seed(seed)
...
model_path = os.path.join(MODEL_DIR, "baseline_resnet50_gtsrb_best.pth")
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "baseline_loss_curve.png"))
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "baseline_accuracy_curve.png"))
plt.close()
...
pd.DataFrame(history).to_csv(
    os.path.join(OUTPUT_DIR, "baseline_training_history.csv"),
    index=False
)
...
pd.DataFrame({
    ...
    os.path.join(OUTPUT_DIR, "baseline_test_metrics.csv"),
    index=False
)

print("Saved baseline_classification_report.txt")
print("Saved baseline_loss_curve.png")
print("Saved baseline_accuracy_curve.png")
print("Saved baseline_confusion_matrix.png")
print("Saved baseline_training_history.csv")
print("Saved baseline_test_metrics.csv")
